...
void MakeDistinctPairs(Node*& headPtr){
    ...
    Node *start = headPtr, *cursor = headPtr->link, *pre = headPtr;    
    int nodeCount = 0;
    ...
    //only one node in list, make copy, attach and return
    if(headPtr->link == 0){
    ...
    while (start != 0){ 
        ...
        while (cursor != 0){
            ...
            if (start->data == cursor->data){ 
                ...
                //2nd duplicate found, delete and link gap
                if(nodeCount > 1){
                    pre->link = cursor->link;
                    delete cursor;
                    if(cursor->link != 0)
               ...
               //move pre and cursor pointers up one 
               pre = cursor;
               cursor = cursor->link;
            }
        
            //no duplicate found, make one and attach
            if (nodeCount == 0){ 
                Node *newNode = new Node;
